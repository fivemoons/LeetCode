...
public class Q18_4Sum {
	public static List<List<Integer>> fourSum(int[] nums, int target) {
        ...
        Map<Integer,List<Integer[]>> map = new HashMap<>(); //用来缓存两个数的和，key：两个数的和 value：两个数下标的list
        
        for(int i=1; i<nums.length; i++){//这是特殊情况。当答案的四个数的后两个相同的时候，加入map
        	if ((nums[i-1] == nums[i])&& //n3 = n4
        			((i+1==nums.length) || (i+1<nums.length)&&(nums[i]!=nums[i+1]))){//a==b!=c
        		List<Integer[]> tmp = new ArrayList<Integer[]>();
        		tmp.add(new Integer[]{i-1,i});
        		map.put(nums[i-1] + nums[i], tmp);
        	}
        }
        for(int i=0; i<nums.length; i++){
        	...
        	for(int j=i+1; j<nums.length; j++){
        		...
        		List<Integer[]> tmp = map.get(nums[i] + nums[j]); //在map中找到和为 n3 + n4 的list
        		if (tmp==null){ //如果为空
        			tmp = new ArrayList<Integer[]>(); //创建新list
        			tmp.add(new Integer[]{i,j}); //添加n3 n4下标进入
        			map.put(nums[i] + nums[j], tmp); //缓存在map中
        		}else{
        			tmp.add(new Integer[]{i,j}); //不为空，说明已经存在一组和为 n3+n4，直接在list后面追加
        		}
        	}
        }
        //为了输出map
